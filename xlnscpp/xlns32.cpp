...
#else
   // define the xlns type and the signed version
  typedef unsigned int xlns32;
  typedef signed int xlns32_signed;

//   define constants to be used in cacculatinos
...
// utility to extract information form xlns type
#define xlns32_sign(x)  ((x) & xlns32_signmask) // sign of the number
#define xlns32_neg(x)   ((x) ^ xlns32_signmask) // negate teh number
#define xlns32_abs(x)   ((x) & xlns32_logmask) // get hte absolute valeu
#define xlns32_recip(x) (xlns32_sign(x)|xlns32_abs((~x)+1)) // 
#define xlns32_sqrt(x)   (xlns32_abs(((xlns32_signed)((x)<<1))/4)^xlns32_sqrtmask) // sqrt of the type
#define xlns32_canon(x) ((x)^(-((x)>>xlns32_canonshift)|xlns32_signmask))// dont know what is this
